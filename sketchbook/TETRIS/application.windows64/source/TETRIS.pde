...
void back()
{
  ...
  float vy=s;
  image(HOLD, s/2, s+vy);
  rect(s/4, 2.5*s+vy, s*n/1.1, s*n/1.5);
  fill(255, 255);
  if(hold>0)
  {
...
void line_check1()
{
  ...
  if (removed_linecnt>0)clear_delay=int(fps/50);
}
...
void draw()
{
  ...
  boolean ppp=true;
  if (clear_delay>0){line_check2();ppp=false;}
  back();
  if(ppp)
  {
    line_check1();
